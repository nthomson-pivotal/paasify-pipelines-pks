...
output "pks_subnet_cidrs" {
  value = element(
    concat(google_compute_subnetwork.pks-subnet.*.ip_cidr_range, [""]),
    0,
  )
}
...
output "pks_master_node_service_account_key" {
  value = base64decode(
    google_service_account_key.pks_master_node_service_account_key.private_key,
  )
}

output "pks_worker_node_service_account_key" {
  value = base64decode(
    google_service_account_key.pks_worker_node_service_account_key.private_key,
  )
}

output "api_endpoint" {
  value = "api.${replace(
    replace(google_dns_record_set.wildcard-pks-dns.name, "/^\\*\\./", ""),
    "/\\.$/",
    "",
  )}"
}
